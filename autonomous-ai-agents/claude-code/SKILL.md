...
## Third-Party Anthropic-Compatible Providers (Kimi, OpenRouter, etc.)

Claude Code can be pointed at third-party APIs that speak the Anthropic Messages protocol. This requires careful configuration.

### Required Environment Variables

```bash
ANTHROPIC_API_KEY="your-third-party-key"
ANTHROPIC_BASE_URL="https://api.provider.com"  # ⚠️ NO trailing /v1
```

**CRITICAL:** Do NOT include `/v1` in `ANTHROPIC_BASE_URL`. The SDK auto-appends it. Including `/v1` produces `/v1/v1` paths, causing 404s on models and messages endpoints:

- ✅ `ANTHROPIC_BASE_URL="https://api.kimi.com/coding"` → SDK calls `/v1/messages` ✓
- ❌ `ANTHROPIC_BASE_URL="https://api.kimi.com/coding/v1"` → SDK calls `/v1/v1/messages` ✗

The error message is misleading: "There's an issue with the selected model (claude-sonnet-4-6). It may not exist or you may not have access to it." — this is actually the `/v1/models/{id}` endpoint returning 404 due to the doubled `/v1/v1` path.

### Model Name Validation

Claude Code fetches `/v1/models/{model-id}` during startup. If your provider doesn't return Anthropic model IDs:
- Use model aliases that Claude Code recognizes: `sonnet`, `opus`, `haiku`, or full names like `claude-sonnet-4-6`, `claude-opus-4-8`
- The provider must either return these IDs from its models endpoint OR map them to its own models internally (Kimi does server-side mapping)
- If the provider only returns its own model IDs (e.g., `kimi-for-coding`), use the Anthropic-standard model name with `--model` and let the provider handle mapping

### Kimi For Coding Specific Setup

```bash
# Install (may need SSL workaround)
...
npx claude -p "your task" --model claude-sonnet-4-6
```

Kimi's model is `kimi-for-coding` but Claude Code won't accept that name. Use Anthropic model names (`claude-sonnet-4-6`, `claude-opus-4-8`) — Kimi maps them server-side.

### Incompatible Tools

- **OpenCode** (opencode-agent): Not on Kimi's allowlist. Kimi only permits: Claude Code, Kimi CLI, Roo Code, Kilo Code.
- **Direct curl/OpenAI SDK**: Kimi's coding endpoint rejects non-coding-agent User-Agents for the OpenAI-compatible path.

### NPM Certificate Issues

Behind corporate proxies or on restricted networks:
```bash
npm config set strict-ssl false  # Workaround for cert errors
npm install -g @anthropic-ai/claude-code
npx claude --version  # Verify: npx claude works even if `claude` isn't in PATH
```
